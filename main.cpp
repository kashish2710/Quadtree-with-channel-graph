...
// };
    ...
    while (true) {
        ...
        if (cmd == "Exit") break;
            ...
            vector<int>path;
           auto start = chrono::high_resolution_clock::now();
           tree->SearchPoint(point_x,point_y,path,graph);
             auto end = chrono::high_resolution_clock::now();
              chrono::duration<double> elapsed = end - start;
           ...
           tree->PathToRoot( point_x, point_y,path,graph);
        }
        else if (cmd == "RectQuery") {
            int x_top, y_top, x_bottom, y_bottom;cin >> x_top >> y_top >> x_bottom >> y_bottom;
             auto start = chrono::high_resolution_clock::now();
            auto res = tree->RectQuery(x_top,y_top,x_bottom,y_bottom,graph);
          ...
          auto res=tree->NetIntersect(x1, y1, x2, y2, graph);
            for (const auto& r : res) cout << r << " ";
            cout << "\n";
        }
    }
